...
Before accepting `Design`, `Self-Review`, `Domain update`, or Promotion Check as complete, verify that the required artifacts for the declared project size exist on disk. If a required artifact is missing, reject completion as `Process Failure: missing required artifact(s)`. If the verification log itself is missing, require it to be written before any further gate recording or phase-complete claim. See `BUILDER.md → Artifact Existence Gate`.
...
- **Atomic profile reconciliation:** When a gate `FAIL` or `BLOCKED` provides reusable evidence for an existing pitfall, you MUST update `occurrence_count`, `Confidence`, and `Source` in the active domain profile in the same session. This is a local reconciliation step, not a promotion decision: it does NOT depend on cross-project reoccurrence, `occurrence_count` thresholds, or any other catalog-promotion criterion. A single same-session `BLOCKED` that proves the pitfall's reusable constraint is sufficient to upgrade local `Confidence` to `confirmed` with a `Source` pointing at the exact gate section. Declining to reconcile because evidence is "only single-session" imports a promotion criterion into a reconciliation duty and is a `Process Failure`. If the verification log shows stronger runtime evidence than the profile metadata at Self-Review time, reject Self-Review as `Process Failure: unreconciled artifacts`. Catalog promotion still follows the failure-backed rule in `BUILDER.md` and `catalog/README.md` — that evaluation happens later, in the Promotion Check, and is independent of whether local reconciliation already raised `Confidence` to `confirmed`.
